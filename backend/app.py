...
def compress_image(image_path, reduction_percentage):
    ...
    try:
        ...
        for i in range(num_seams):
            ...
            progress = ((i + 1) / num_seams) * 50  # 50% of progress for seam removal
            socketio.emit('progress', {'progress': progress})

        # Downscale if necessary
        if scale_factor < 1.0:
            img = downscale_image(img, scale_factor)
            socketio.emit('progress', {'progress': 75})  # 75% of progress after downscaling

        # Post-process with bilateral denoising
        img = post_process(img, 75)  # Start post-processing at 75%
        socketio.emit('progress', {'progress': 100})  # 100% after post-processing
